...
impl LoginState {
    ...
    pub fn update(&mut self, _dt: f32) {
        ...
        // Calculate actual draw position accounting for origin (same as in draw())
        let (username_field_x, username_field_y, username_width, username_height) =
            if let Some(field) = &self.maple_id_label {
                ...
                (draw_x, draw_y, field.texture.width(), field.texture.height())
            } else {
                (username_center_x, username_center_y, 200.0, 30.0)
            };
        ...
        // Calculate actual draw position accounting for origin (same as in draw())
        let (password_field_x, password_field_y, password_width, password_height) =
            if let Some(field) = &self.password_label {
                ...
                (password_center_x, password_center_y, 200.0, 30.0)
            };
        ...
        if self.focused_field != FocusedField::None {
            // Handle backspace separately to support holding it down
            ...
            // Get all pressed keys this frame for character input
    ...
    pub fn draw(&self) {
        ...
        // Calculate draw positions accounting for origin
        let (username_draw_x, username_draw_y) = if let Some(field) = &self.maple_id_label {
            (username_center_x - field.origin.x, username_center_y - field.origin.y)
        } else {
            (username_center_x, username_center_y)
        };

        // Draw MapleID texture only when field is empty
        // When field has text, show the input text instead
        let show_username_texture = self.username.is_empty();

        if show_username_texture {
            // Draw MapleID input field texture
            if let Some(field) = &self.maple_id_label {
                draw_texture(&field.texture, username_draw_x, username_draw_y, WHITE);
            ...
            // Draw username text input (when focused or has content)
            // Position text in same location as where the texture would be drawn
            let font_size = 16.0;
            let texture_height = self.maple_id_label.as_ref().map(|t| t.texture.height()).unwrap_or(30.0);
            let text_y = username_draw_y + (texture_height / 2.0) + (font_size / 3.0);
            ...
            if self.focused_field == FocusedField::Username {
                ...
                let cursor_x = username_draw_x + 5.0 + text_width;
                draw_line(cursor_x, text_y - font_size * 0.75, cursor_x, text_y + font_size * 0.25, 2.0, BLACK);
            }
        }

        // Calculate draw positions accounting for origin
        let (password_draw_x, password_draw_y) = if let Some(field) = &self.password_label {
            (password_center_x - field.origin.x, password_center_y - field.origin.y)
        } else {
            (password_center_x, password_center_y)
        };

        // Draw PW texture only when field is empty
        // When field has text, show the input text instead
        let show_password_texture = self.password.is_empty();

        if show_password_texture {
            // Draw PW input field texture
            if let Some(field) = &self.password_label {
                draw_texture(&field.texture, password_draw_x, password_draw_y, WHITE);

                // Debug: Draw a small red dot at the center position to help with alignment
                draw_circle(password_center_x, password_center_y, 3.0, RED);
            }
        } else {
            // Draw password text input (masked, when focused or has content)
            // Position text in same location as where the texture would be drawn
            let password_masked: String = self.password.chars().map(|_| '*').collect();
            let font_size = 16.0;
            let texture_height = self.password_label.as_ref().map(|t| t.texture.height()).unwrap_or(30.0);
            let text_y = password_draw_y + (texture_height / 2.0) + (font_size / 3.0);
            ...
            if self.focused_field == FocusedField::Password {
                ...
                let cursor_x = password_draw_x + 5.0 + text_width;
                draw_line(cursor_x, text_y - font_size * 0.75, cursor_x, text_y + font_size * 0.25, 2.0, BLACK);
            }
        }
